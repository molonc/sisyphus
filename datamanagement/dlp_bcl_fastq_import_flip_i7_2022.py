...
def get_samplesheet(destination, lane_id):
    def reverse_complement_line(line_in):
        def reverse_complement(target_string):
            complement = {'A': 'T', 'C': 'G', 'G': 'C', 'T': 'A'} 
            return ''.join([complement[char_i] for char_i in target_string[::-1]])
        return re.sub(r'([GACT]{6})', lambda x:reverse_complement(x.group()),line_in, count=1)
    ...
    with open(destination, 'w+') as f:
        sample_sheet_in = r.content.decode("utf-8").split("\n")
        sample_sheet_out = [reverse_complement_line(line_n) for line_n in sample_sheet_in]
        sample_sheet_out = "\n".join(sample_sheet_out)
        f.write(sample_sheet_out)
...
def prep_slurm_submission(flowcell_id, bcl_dir, output_dir):
    """
    Temporary solution; Prepare for SLURM submission
    """
    if len(os.listdir(output_dir)) > 0:
        raise Exception('bcl2fastq output directory {} is not empty'.format(output_dir))
    ...
    get_samplesheet(samplesheet_filename, flowcell_id)
    make_slurm_script(flowcell_id, bcl_dir, samplesheet_filename, output_dir)

    logging.info("Submit the generated script to run bcl2fastq.")

def make_slurm_script(flowcell_id, bcl_dir, samplesheet_path, output_dir, cores=16):      # add sl
    slurm_script_path = os.path.join(output_dir, "submit.sh")

    bcl2fastq_cmd = [
        '/gsc/software/linux-x86_64-centos7/bcl2fastq2-2.20/bin/bcl2fastq',
        '--processing-threads',
        str(cores),
        '--runfolder-dir',
        bcl_dir,
    ...
    with open(slurm_script_path, 'w') as fh:
        fh.write("#!/bin/bash\n")
        fh.write("#SBATCH --job-name=BCL2FASTQ\n")
        fh.write("#SBATCH --time=72:00:00 \n") # give 3 days to complete
        fh.write("#SBATCH --cpus-per-task=16") # 16 parallel core
...
def main(
    ...
    #    )
    if not no_bcl2fastq:
        prep_slurm_submission(flowcell_id, bcl_dir, temp_output_dir)
    else:
        # Import fastqs
        load_brc_fastqs(
            flowcell_id,
