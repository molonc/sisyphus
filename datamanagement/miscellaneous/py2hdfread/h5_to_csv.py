...
def compare_df(df0,df1):
    ...
    df0.fillna(value=0, inplace=True) # probably will fail if nan-s are in non numeric fields
    df1.fillna(value=0, inplace=True)

    #numeric = [float64','int64'] # in pandas bool is not subclass of int
    #l = np.allclose(df0.select_dtypes(include=numeric), df1.select_dtypes(include=numeric))
    #logger.info('tested: numeric; l=%i;' % (l))
...
def read_csv_with_types(filename):
    ...
    Returns:
        DataFrame
    """
    fnm = filename[:-len(csv)] if filename.endswith(csv) else filename[:-len(suff_new)]
...
def write_csv_with_types(df,filename,index=False,encoding='utf-8'):
    ...
    """
    csv = '.csv'
    yaml = '%s.yaml' % (filename[:-len(csv)] if filename.endswith(csv) else filename[:-len(csv+'.gz')])
...
def convert1(hdf5_filename):
    ...
    """
    #file = '/dat/tmp/SC-1130/_alignment_metrics.h5'
    #file = '/Users/ogolovko/w/bccrc/tt/_hmmcopy.h5'
    ...
    for k in keys:
        ...
        #assert (df.equals(df_test)) #(df == df_test).all().all()
        with Timeit('compare; {}'.format(csv_filename)):
            #print(assert_frame_equal(df, df_test, check_dtype=False))
            assert(compare_df(df, df_test))
        #show_diff(df,df_test)
...
def compare_df_test(isOk,a,b,msg):
    ...
    logging.info('df_test();\ntesting {};\n{};\n{};      res={}; {}'.format(msg,dfa,dfb,l,'OK' if res else 'ERROR'))
    return res
