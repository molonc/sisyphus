...
class GSC_Import(object):
	def __init__(
		...
		try:
			...
			raise ValueError(f"Could not connect to GSC API. See {err} for detail.")

		self.gsc_internal_id = gsc_internal_id
		self.colossus_api = ColossusApi()
		self.tantalus_api = TantalusApi()

	def upload_blob_async(self, data, storage_account_name, concurrency=3, update=False):
		"""
	    ...
	    Args:
	        ...
	        storage_account_name: name of the Azure Storage Account as stored in Tantalus
		"""
		async_storage_client = tantalus_api.get_storage_client(storage_account_name, is_async=True, concurrency=concurrency)
