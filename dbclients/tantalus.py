...
class TantalusApi(BasicAPIClient):
    ...
    def add_sequence_dataset(
        ...
        for lane in sequence_lanes:
            ...
            lane_library_pk = self.get_or_create(
                "dna_library",
        ...
        if tag_name is not None:
            tag_pk = self.get_or_create("tag", name=tag_name)["id"]
            tags = [tag_pk]
        else:
            tags = []
        ...
        for file_path in file_paths:
            file_resource, file_instance = self.add_file(storage_name, file_path, update=update)
            file_resource_pks.append(file_resource["id"])

        # Create the sequence dataset associated with the above data
        try:
            sequence_dataset = self.get(
                    "sequence_dataset",
                    name=dataset_name,
