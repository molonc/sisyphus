chasmbrain <- function(input) {
  ...
  col_order =c("cells_LQ","cells_MID","cells_PASS","cells_WOW","reads_LQ","reads_MID","reads_PASS","reads_WOW","total_reads","total_cells")
  for (l in col_order){
    if(!(l %in% colnames(bins_stats2)))
    {
      bins_stats2[l] <- 0;
    }
  }
  bins_stats2 <- bins_stats2[,col_order]
