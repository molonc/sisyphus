...
def generate_loader_command(jira, project_args, es_host="10.1.0.8"):
    ...
    if(project_args):
        command = f'bash /home/spectrum/alhena-loader/load_ticket.sh {jira} {es_host} " --reload {project_args}"'
    else:
...
def run_viz_alhena(
    ...
    analyses = colossus_api.list("analysis_information", library__pool_id='A118375B')
    failed = []
    for analysis in analyses:
        # get library id
        library_id = analysis["library"]["pool_id"]
...
analysis = tantalus_api.list(
    "file_resource",
    id=220498,
)
b = [i for i in analysis]
...
tantalus_api.update(
    "file_resource",
    id=220498,
    filename="SA919X1/illumina_wtss/A60780/bwa-mem/bwa-mem-0.7.6a/C9W6RANXX_2_CGATGT_withJunctionsOnGenome_dupsFlagged.bam"
)

new_path = "data/single_cell_indexing/bam/A60780/lanes_32effa05/SA919X1_A60780_32effa05.bam"
"SA919X1/illu
...
analysis = tantalus_api.list(
    "file_instance",
    file_resource__in=220498,
)
b = [i for i in analysis]



analysis = tantalus_api.update(
    "file_instance",
    file_resource__in=220498,
    storage=
)
...
python transfer_files.py transfer 8147 sequencedataset shahlab singlecellblob